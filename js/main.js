...
let compareToken = 0;             // superseded compares must not paint over the newest one
...
const resultsApi = initResults(document.getElementById('results'), {
  ...
  onRetry: (profile) => retryProfile(profile),
  onSortChange: (key) => {
...
async function runCompare() {
  ...
  // Snapshot at start (not on success) so returning to the main screen mid-compute with
  // unchanged settings doesn't trigger a second compare that aborts this one.
  lastComputeKey = JSON.stringify(settings);
  actions.beginRouting();

  try {
    const results = await computeAllModes(s.start, s.end, settings, { signal: abort.signal });
    if (token !== compareToken) return;   // superseded by a newer compare — discard
...
// Re-route a single failed mode without touching the cards that worked. Wired to the
// "Try again" button on unavailable cards.
async function retryProfile(profile) {
  const s = getState();
  if (!s.start || !s.end || !currentResults || s.status !== 'results') return;
  currentResults = { ...currentResults, [profile]: { retrying: true } };
  setState({ results: currentResults });
  const partial = await computeAllModes(s.start, s.end, settings, { profiles: [profile] });
  if (getState().status !== 'results') return;   // a full re-compare took over meanwhile
  currentResults = { ...currentResults, ...partial };
  setState({ results: currentResults });
