...
export class OsrmRouter {
  ...
  async route(start, end, profile, opts = {}) {
    ...
    // Shorter-than-default timeout: when this instance stalls (it happens per-profile
    // under load), we want the Valhalla fallback to engage before the user gives up.
    const json = await httpGetJson(url, { signal: opts.signal, timeoutMs: 8_000 });
