 ...
 * @param {{ signal?: AbortSignal, profiles?: string[] }} [opts] — profiles narrows the
 *   run to a subset (used by the per-card "Try again" retry).
 * @returns {Promise<Object>} { car: ModeResult|{error}, bicycle: ..., ... }
 */
export async function computeAllModes(start, end, settings, opts = {}) {
  const tasks = (opts.profiles || PROFILES).map(async (profile) => {
