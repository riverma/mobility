...
export function initResults(rootEl, { onSelect, onSortChange, onRetry }) {
  ...
  function renderCard(profile, r, { isSelected, isBest, units, includeMaintenance }) {
    ...
    if (!r || r.error || r.skipped || r.retrying) {
  ...
  function renderUnavailableCard(profile, r) {
    ...
    fallback.appendChild(el('p', { class: 'mc-card-sub' },
      r?.retrying ? 'Retrying…' :
      r?.error ? friendlyError(r.error) :
      r?.skipped ? `Not available in v1 (${r.reason})` :
      'No data.'));
    if (r?.error && onRetry) {
      const btn = el('button', { class: 'mc-retry-btn', type: 'button' }, 'Try again');
      btn.addEventListener('click', () => onRetry(profile));
      fallback.appendChild(btn);
    }
    return fallback;
  }

  // Raw fetch failures ("timeout", "Fetch is aborted", "Failed to fetch") mean nothing
  // to users — translate the transport-level ones; keep anything specific (e.g. "no
  // route found") as-is.
  function friendlyError(msg) {
    if (/timeout|abort|failed to fetch|networkerror|load failed/i.test(msg)) {
      return 'The routing server didn’t respond.';
    }
